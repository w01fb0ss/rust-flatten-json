...
fn flatten_object(
    ...
    for (k, v) in nested_dict.iter() {
        ...
        if let Some(obj) = v.as_object() {
            ...
            if !v_array.is_empty() {
                // traverse array
                if let Some(value) = flat_value.as_object_mut() {
                    value.insert(new_k, json!(v_array));
                };
...
// fn infer_type_and_insert_array(v: &mut Value, new_k: String, value: &[Value]) -> Result<()> {
//     v.insert(new_k, value);
//     Ok(())
// }
